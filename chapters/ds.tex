...
Assuming that all priorities are distinct, then $x_i$ is an ancestor of $x_j$ in $T$ if and only if $p_i \geq p_h$ for all $i \leq h \leq j$.
\end{lem}
\begin{proof}
...
An immediate consequence of the ancestor lemma is the common ancestor lemma:
...
The harmonic numbers are given by $H_n = \sum_{i=1}^n \frac{1}{i}$ and they satisfy
\[
    \ln n < H_n < 1 + \ln n
\]
...
\todo{Argue that search, insert and delete have the required bounds}
