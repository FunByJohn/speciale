...
\end{figure}
The diagram consists of line segments and vertices. Now, a natural question arises: how do we store Voronoi diagrams on a computer? 
...
We'll need the following geometric data structure:
...
\[
    \includegraphics[scale=0.25]{temp-fig-9}
\]
...
\]
edges.
\todo{This is wrong. If we have a diagram with only 2 sites and 2 edges, we can construct it in such a way that it has 7 edges, contradicting the upper bound $4n - 2$. It's instead the case that every cell may get at most 4 new edges, one for each side of the bounding box.}
\end{rmk}
